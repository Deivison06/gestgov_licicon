...
@section('page-subtitle', 'Selecione um contrato para registrar a ocorrência')
...
<div x-data="{
    ...
    <div class="mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-5 space-y-4">
            @if(!$isPrefeituraUser)
                <form method="GET" action="{{ route('admin.ocorrencias.selecionar-contrato') }}" class="max-w-xs">
                    ...
                    <div class="p-4 space-y-2.5">
                        @foreach($empresa['contratos'] as $contrato)
                            <div x-show="matchContrato({{ json_encode($contrato) }})"
                                 class="bg-white p-4 rounded-lg border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 hover:shadow-md hover:border-[#009496]/30 transition-all">
                                ...
                                <div class="flex items-center gap-4 shrink-0">
                                    @if($contrato['ultima_ocorrencia'])
                                        <div class="flex items-center gap-2 pr-4 border-r border-gray-100">
                                            <a href="{{ route('admin.ocorrencias.show', $contrato['ultima_ocorrencia_id']) }}"
                                               class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Ver Última Ocorrência">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                        </div>
                                    @endif

                                    <a href="{{ route('admin.ocorrencias.create', ['id' => $contrato['id'], 'type' => $contrato['type']]) }}"
