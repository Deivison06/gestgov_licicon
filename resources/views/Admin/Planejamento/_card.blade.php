...
<div data-id="{{ $processo->id }}"
    ...
    <div class="p-3 flex flex-col gap-2.5 flex-1">
        ...
        <div class="flex flex-col gap-1.5 mt-auto pt-1"
            ...
            @if($processo->planejamento_status === 'em_elaboracao')
                <button @click="$dispatch('abrir-modal-agendar', { id: {{ $processo->id }}, dataSessao: '{{ $processo->detalhe?->data_hora_fase_edital?->toDateString() ?? '' }}', horaSessao: '{{ $processo->detalhe?->data_hora_fase_edital?->format('H:i') ?? '' }}' })"
