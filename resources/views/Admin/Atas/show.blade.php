...
@section('content')
    <div class="py-6">
        ...
        <div id="aba-contratacoes" class="tab-content hidden">
            ...
            @else
                <!-- Adicione este botão no topo da seção de contratações -->
                <div class="mb-6">
                    <div class="flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900">Contratações Pendentes</h3>
                            <p class="text-sm text-gray-600">
                                Total: {{ $contratacoes->sum(function ($group) {return $group->count();}) }} contratações
                            </p>
                        </div>
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        <div class="text-right">
                                            ...
                                            <p class="font-bold text-green-600">
                                                R$
                                                {{ number_format($vencedorContratacoes->sum('valor_total'), 2, ',', '.') }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
`;
                    ...
                    .then(data => {
                        ...
                        `;
                            ...
                            if (data.lotes && data.lotes.length === 0) {
                                mensagem =
                                    'Todos os itens deste vencedor já foram contratados ou não há quantidade disponível.';
                            }
        ...
        async function editarContratacao(contratacaoId) {
            try {
                const response = await fetch(`/admin/atas/${processoId}/contratacao/${contratacaoId}/edit`);
                const data = await response.json();

                if (data.success) {
                    abrirModalEdicaoContratacao(data.contratacao, data.estoque, data.max_quantidade);
                } else {
                    mostrarMensagem('Erro ao carregar dados para edição: ' + data.message, 'error');
                }
            } catch (error) {
                console.error('Erro:', error);
        ...
        // Função para abrir modal de edição
        function abrirModalEdicaoContratacao(contratacao, estoque, maxQuantidade) {
            const modalHtml = `
                ...
                <div class="px-6 py-4 border-b">
                    <div class="flex justify-between items-center">
                        <h3 class="text-lg font-bold text-gray-900">
                            Editar Contratação - ${contratacao.item}
                        </h3>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span class="font-medium ml-2">${contratacao.item}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Descrição:</span>
                                <span class="font-medium ml-2">${contratacao.descricao}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Valor Unitário:</span>
                                <span class="font-medium ml-2">
                                    R$ ${parseFloat(contratacao.valor_unitario).toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                </span>
                            </div>
                            <div>
                                <span class="text-gray-500">Disponível no Estoque:</span>
                                <span class="font-medium ml-2">
                                    ${parseFloat(maxQuantidade).toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="number"
                                ...
                                oninput="calcularValorTotalEdicao(${contratacao.valor_unitario})">
                        ...
                        <div class="p-4 bg-blue-50 rounded-lg">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="font-bold" id="quantidade_display">
                                        ${parseFloat(contratacao.quantidade_contratada).toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                    </div>
                                </div>
                                <div class="col-span-2">
                                    <span class="text-sm text-gray-600">Valor Total:</span>
                                    <div class="font-bold text-green-600 text-lg" id="valor_total_display">
                                        R$ ${parseFloat(contratacao.valor_total).toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                    </div>
                                </div>
                            </div>
    ...
    `;
            ...
            // Criar e adicionar modal ao DOM
            const modalDiv = document.createElement('div');
            modalDiv.id = 'modal-edicao-contratacao';
            modalDiv.innerHTML = modalHtml;
            document.body.appendChild(modalDiv);

            // Inicializar cálculo do valor total
            calcularValorTotalEdicao(contratacao.valor_unitario);
        }

        // Função para calcular valor total na edição
        function calcularValorTotalEdicao(valorUnitario) {
            const quantidadeInput = document.getElementById('quantidade_edit');
            const quantidadeDisplay = document.getElementById('quantidade_display');
            const valorTotalDisplay = document.getElementById('valor_total_display');

            const quantidade = parseFloat(quantidadeInput.value) || 0;
            const valorTotal = quantidade * parseFloat(valorUnitario);

            quantidadeDisplay.textContent = quantidade.toLocaleString('pt-BR', {
                minimumFractionDigits: 2
            });
            valorTotalDisplay.textContent = 'R$ ' + valorTotal.toLocaleString('pt-BR', {
                minimumFractionDigits: 2
            });
        }

        // Função para salvar edição
        async function salvarEdicaoContratacao(contratacaoId) {
            const quantidade = document.getElementById('quantidade_edit').value;
            const observacao = document.getElementById('observacao_edit').value;
            ...
            try {
                ...
                const response = await fetch(`/admin/atas/${processoId}/contratacao/${contratacaoId}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Content-Type': 'application/json',
                ...
                if (data.success) {
                    mostrarMensagem('Contratação atualizada com sucesso!', 'success');
                    fecharModalEdicao();

                    // Atualizar a tabela de contratações
                    await atualizarAbaContratacoes();
                    ...
                    mostrarMensagem('Erro ao atualizar: ' + data.message, 'error');
                }
            } catch (error) {
                console.error('Erro:', error);
        ...
        async function excluirContratacao(contratacaoId) {
            ...
            await excluirContratacaoConfirmar(contratacaoId);
        }

        // Função para confirmar exclusão
        async function excluirContratacaoConfirmar(contratacaoId) {
            try {
                mostrarMensagem('Excluindo contratação...', 'info');
                ...
                if (data.success) {
                    mostrarMensagem('Contratação excluída com sucesso!', 'success');
                    ...
                    mostrarMensagem('Erro ao excluir: ' + data.message, 'error');
                }
            } catch (error) {
                console.error('Erro:', error);
                mostrarMensagem('Erro ao excluir contratação', 'error');
            }
        }
        ...
        async function excluirTodasContratacoes() {
            ...
            try {
                mostrarMensagem('Excluindo todas as contratações pendentes...', 'info');

                const response = await fetch(`/admin/atas/${processoId}/excluir-todas-contratacoes`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Content-Type': 'application/json',
                ...
                console.error('Erro ao excluir todas as contratações:', error);
                mostrarMensagem('Erro ao excluir todas as contratações', 'error');
            }
        }
    ...
    <style>
        ...
        #modal-contratacao .overflow-y-auto {
            ...
            /* Ajuste baseado na altura dos cabeçalhos/rodapés */
        }
