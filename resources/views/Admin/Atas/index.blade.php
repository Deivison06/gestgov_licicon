            ...
            <div class="mb-6">
                <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <form method="GET" action="{{ route('admin.atas.index') }}" class="space-y-4">
                            ...
                            <div class="mt-1">
                                <div class="relative">
                                    ...
                                    <input type="text"
                                           ...
                                           id="search"
                                           value="{{ $search ?? '' }}"
                                           placeholder="Pesquise por objeto, número do processo, nome do contratado..."
                                ...
                                <p class="mt-1 text-xs text-gray-500">
                                    Busca por: parte do objeto, número do processo/procedimento, nome do contratado ou prefeitura
                                </p>
                            </div>
                        </div>
                        ...
                        @if($prefeituraId || $processoId || !empty($search))
                            <div class="pt-4 mt-4 border-t border-gray-200">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if($prefeituraId)
                                        @php
                                            $prefeituraSelecionada = $prefeituras->find($prefeituraId);
                                        @endphp
                                    ...
                                    Prefeitura: {{ $prefeituraSelecionada->cidade }}
                                </span>
                                        @endif
                                    @endif
                                    ...
                                    @if($processoId)
                                        @php
                                            $processoSelecionado = $processos->find($processoId);
                                        @endphp
                        ...
                        <div>
                            ...
                            @if(!empty($search))
                                <p class="text-xs text-gray-600">
                                    Resultados da pesquisa por: "<strong>{{ $search }}</strong>"
                                </p>
                            @endif
                        </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    @foreach($processos as $processo)
                                        @php
                                            ...
                                            // Determinar classe da badge de modalidade
                                        ...
                                        <!-- Segunda linha: objeto (com colspan) -->
                                        <tr class="bg-gray-50">
                                            <td colspan="7" class="px-6 py-4 text-sm text-gray-700">
                                                <div class="space-y-1">
                                                    <div>
                                                        <strong class="text-gray-900">Objeto:</strong> {!! strip_tags($processo->objeto) !!}
                                                    </div>
                                                    @if($processo->user)
                                                        <div class="text-xs text-gray-500">
                                                            ...
                                                            Responsável: {{ $processo->user->name }}
                                                        </div>
                                                    @endif
                                                    @if($processo->vencedores->count() > 0)
                                                        <div class="text-xs text-gray-600">
                                                            ...
                                                            @foreach($processo->vencedores as $vencedor)
                                                                <span class="font-medium">{{ $vencedor->nome ?? $vencedor->razao_social }}</span>
                                                                @if(!$loop->last), @endif
                                                            @endforeach
                                                        </div>
                                                    @endif
                    ...
                    @else
                        ...
                        <div class="py-12 text-center">
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900">
                                ...
                                @else
                                    Nenhum processo disponível para atas
                                @endif
                            </h3>
                            <p class="mt-2 text-sm text-gray-500">
                                @if(!empty($search))
                                    Nenhum resultado encontrado para a pesquisa "{{ $search }}"
                                @elseif($prefeituraId || $processoId)
                                    Tente ajustar os filtros ou limpar para ver todos os processos
                                @else
                                    Todos os processos disponíveis já foram concluídos ou não possuem contratações pendentes
                                @endif
                            </p>
                            <div class="mt-6">
                                <a href="{{ route('admin.atas.index') }}"
