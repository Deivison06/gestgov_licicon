        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function renderizarResultadosPncp(data) {
                ...
                data.data.forEach(item => {
                    html += `
                    <div class="group p-4 bg-white border border-gray-100 rounded-2xl hover:border-blue-200 hover:shadow-md transition-all cursor-pointer relative overflow-hidden" 
                        onclick="verItensPncp('${item.orgaoEntidade.cnpj}', '${item.anoCompra}', '${item.sequencialCompra}')">
                        <div class="absolute left-0 top-0 bottom-0 w-1 bg-transparent group-hover:bg-blue-600 transition-all"></div>
                        <div class="flex justify-between items-start gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1.5">
                                    <span class="px-2 py-0.5 bg-blue-50 text-blue-600 text-[9px] font-bold rounded uppercase tracking-wider">${item.modalidadeNome}</span>
                                    <span class="text-[10px] font-bold text-gray-400">#${item.sequencialCompra}/${item.anoCompra}</span>
                                </div>
                                <h5 class="font-bold text-gray-800 text-sm mb-1 group-hover:text-blue-700 transition-colors">${item.orgaoEntidade.razaoSocial}</h5>
            ...
            function renderizarItensPncp(itens) {
                ...
                let html = `
                    <div class="flex items-center justify-between mb-4 px-1">
                        <button onclick="executarBuscaPncp()" class="inline-flex items-center text-xs font-bold text-blue-600 hover:text-blue-800 transition-colors uppercase tracking-wider">
