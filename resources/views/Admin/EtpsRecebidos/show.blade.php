...
@section('content')
        ...
        <div class="bg-white border border-gray-100 shadow-sm rounded-2xl overflow-hidden mb-8">
            <div
                ...
                <h3 class="text-xl font-semibold text-gray-800">
                    <i class="fas fa-file-alt mr-2 text-[#009496]"></i>
                ...
                <div class="flex space-x-2">
                    @if (in_array($etp->status, ['pendente', 'em_analise', 'recusado']))
                        <form action="{{ route('admin.etps_recebidos.status', $etp->id) }}" method="POST" class="inline"
                            onsubmit="return confirm('Confirmar aprovação deste ETP?');">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="status" value="aprovado">
                            <button type="submit"
                                class="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-all flex items-center shadow-sm">
            ...
            <div class="p-8 border-b border-gray-100">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <ul class="space-y-4">
                            <li>
                                <span
                                    class="text-gray-500 font-medium block text-xs uppercase tracking-wider mb-1">Prefeitura
                                    / Licitante</span>
                                <span class="text-gray-900 font-bold text-base">{{ $etp->prefeitura->nome ?? 'N/A' }}</span>
                            </li>
                            <li>
                                <span
                                    class="text-gray-500 font-medium block text-xs uppercase tracking-wider mb-1">Secretaria
                                    Solicitante</span>
                                <span class="text-gray-900 font-semibold">{{ $etp->secretaria->nome ?? 'N/A' }}</span>
                            </li>
                            <li>
                                <span
                                    class="text-gray-500 font-medium block text-xs uppercase tracking-wider mb-1">Responsável
                                    pela Solicitação</span>
                            ...
                            <li class="pt-3 border-t border-gray-200">
                                ...
                                <span
                                    class="inline-block bg-purple-100 text-purple-800 px-3 py-1 rounded-md font-bold uppercase text-sm">{{ $etp->modalidade }}</span>
                            </li>
                            <li class="pt-3 border-t border-gray-200">
                ...
                @if ($etp->processo_id)
                    <div
                        class="mt-8 bg-purple-50 p-6 rounded-xl border border-purple-200 shadow-sm relative overflow-hidden">
                        <div class="absolute top-0 right-0 -mr-8 -mt-8 opacity-10">
                            <i class="fas fa-link text-9xl text-purple-900"></i>
                        </div>
                        <div class="flex items-center relative z-10">
                            <div
                                class="w-16 h-16 rounded-full bg-purple-600 text-white flex items-center justify-center mr-5 shadow-md">
                                <i class="fas fa-link text-2xl"></i>
                            </div>
                            <div>
                                <h4 class="text-xl font-bold text-purple-900 mb-1">Vinculado a Processo de Licitação</h4>
                                <p class="text-sm text-purple-800 mb-3">Este ETP já foi convertido em um processo de
                                    licitação
                                    ativo.</p>
                                <div class="bg-white px-4 py-2 rounded-lg border border-purple-200 inline-block">
                                    <span
                                        class="text-purple-600 text-xs font-bold uppercase tracking-wider block mb-1">Processo
                                        Nº</span>
                                    <span
                                        class="text-gray-900 font-mono font-bold">{{ $etp->processo->numero_processo ?? $etp->processo_id }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                @else
                    ...
                    <div class="mt-8 pt-8 border-t border-gray-200">
                        <h4 class="text-lg font-bold text-gray-800 mb-4 border-b border-gray-200 pb-2"><i
                                class="fas fa-cogs mr-2 text-indigo-600"></i> Ações do Sistema</h4>
                            ...
                            <div class="bg-indigo-50 border border-indigo-100 p-6 rounded-xl shadow-sm">
                                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <p class="text-sm text-indigo-700">Este ETP está aprovado e pronto para ser
                                            transformado em
                                            um processo formal de licitação.</p>
                                    </div>

                                    <form action="{{ route('admin.etps_recebidos.processo', $etp->id) }}" method="POST"
                                        class="flex-shrink-0" id="linkProcessForm">
                                        @csrf
                                        <input type="hidden" name="processo_id" id="processo_id_input">
                                        <button type="button" onclick="openLinkProcessModal()"
                                            class="px-6 py-3 font-semibold text-white bg-indigo-600 rounded-lg shadow-md hover:bg-indigo-700 hover:shadow-lg transition-all flex items-center transform hover:-translate-y-0.5"
                                            title="Vincular a um novo Processo">
                        ...
                        @else
                            <div class="bg-gray-50 border border-gray-200 p-6 rounded-xl shadow-sm text-center">
                                ...
                                <p class="text-sm text-gray-500 max-w-lg mx-auto">A criação vinculada de um Processo
                                    Licitatório só
                                    estará disponível quando o status deste ETP for alterado para <span
                                        class="font-bold text-green-600 relative bottom-px">Aprovado</span>.</p>
                            </div>
                        @endif
                    </div>
                @endif
    ...
    @if ($etp->status === 'aprovado' && !$etp->processo_id)
        <div id="modal-link"
            class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-60 backdrop-blur-sm transition-opacity">
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col transform transition-all scale-95 opacity-0"
                id="modal-link-content">
                <div class="p-6 border-b border-gray-100 bg-indigo-50 flex justify-between items-center">
                    ...
                    <button onclick="closeLinkProcessModal()"
                        class="text-gray-400 hover:text-red-500 bg-white rounded-full w-8 h-8 flex items-center justify-center shadow-sm focus:outline-none transition-colors duration-200">
                ...
                <div class="p-8">
                    <div class="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-start">
                        ...
                        <p class="text-sm text-blue-800 leading-relaxed font-medium">Ao criar um novo processo a partir
                            deste
                            botão, o sistema redirecionará você para a tela de criação padrão de processos, onde você deve
                            preencher o restante dos dados da licitação pertinentes à Prefeitura
                            <strong>{{ $etp->prefeitura->nome }}</strong>.
                        </p>
                    </div>

                    <p class="text-sm text-gray-700 mb-6 pb-6 border-b border-gray-100 text-center font-medium">
                        Confirma a intenção de iniciar um novo processo para o <strong
                            class="text-gray-900 text-base">ETP-{{ str_pad($etp->id, 4, '0', STR_PAD_LEFT) }}</strong>?
                    </p>

                    <div class="flex justify-center md:space-x-4 flex-col md:flex-row space-y-3 md:space-y-0">
                        <button type="button" onclick="closeLinkProcessModal()"
                            class="px-6 py-2.5 text-sm font-bold text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all shadow-sm w-full md:w-auto">
                            Não, Voltar
                        </button>
                        <button type="button" onclick="submitLinkProcessForm()"
                            class="px-6 py-2.5 text-sm font-bold text-white bg-indigo-600 border border-transparent rounded-lg hover:bg-indigo-700 transition-all shadow-md w-full md:w-auto">
        ...
        <script>
            ...
            function submitLinkProcessForm() {
                ...
                document.getElementById('linkProcessForm').action = "{{ route('admin.processos.create') }}?etp_id=" + etpId +
                    "&prefeitura_id=" + prefId + "&objeto=" + obj + "&modalidade=" + modalidade + "&tipo_contratacao=" +
                    tipoContratacao;
