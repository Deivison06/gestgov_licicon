...
<head>
    <meta charset="UTF-8">
    <title>ATA DE REGISTRO DE PREÇOS - Processo {{ $processo->numero_processo ?? $processo->id }}</title>
    <style type="text/css">
        @font-face {
            font-family: 'Aptos';
...
<body>
    <div>
        ...
        @php
            ...
            $diaExtenso = new NumberFormatter('pt_BR', NumberFormatter::SPELLOUT);
            $diaExtenso = $diaExtenso->format($data->day);

            // Ano por extenso (ex: 2024 → dois mil e vinte e quatro)
            $anoFormatter = new NumberFormatter('pt_BR', NumberFormatter::SPELLOUT);
            $anoExtenso = $anoFormatter->format($data->year);
        @endphp
        <p style="text-align: justify;">
            Aos {{ $diaExtenso }} dias do mês de {{ $data->translatedFormat('F') }} do ano de {{ $anoExtenso }}. na
            {{ $processo->finalizacao->endereco }}, o Município de {{ $processo->prefeitura->cidade }},
        ...
        <p style="text-align: justify;">
            ...
            de {{ $processo->prefeitura->cidade }}, observadas as condições fixadas neste instrumento, no Edital e as
        ...
        <p style="text-align: justify;">
            ...
            Municipal de {{ $processo->prefeitura->cidade }}, depois de requeridos por seus órgãos ou unidades;
            Para firmeza e validade do pactuado, a presente Ata foi lavrada em 4 (quatro) vias de
        ...
        <h4 style="text-align: center;">CADASTRO DE RESERVAS</p>
        @foreach($processo->reservas as $reserva)
            <table style="width:100%; border-collapse:collapse; font-size:11px; margin:20px 0;" border="1">

                <!-- Título -->
                <tr>
                    <td colspan="2"
                        style="font-weight:bold; text-align:center; padding:6px;">
                        INFORMAÇÕES DA EMPRESA
                    </td>
                </tr>

                <tr>
                    <td style="width:25%; padding:6px; font-weight:bold;">RAZÃO SOCIAL</td>
                    <td style="padding:6px;">{{ $reserva->razao_social }}</td>
                </tr>

                <tr>
                    <td style="padding:6px; font-weight:bold;">CNPJ</td>
                    <td style="padding:6px;">{{ $reserva->cnpj }}</td>
                </tr>

                <tr>
                    <td style="padding:6px; font-weight:bold;">ENDEREÇO</td>
                    <td style="padding:6px;">{{ $reserva->endereco }}</td>
                </tr>

                <tr>
                    <td style="padding:6px; font-weight:bold;">TELEFONE</td>
                    <td style="padding:6px;">{{ $reserva->telefone }}</td>
                </tr>

                <tr>
                    <td style="padding:6px; font-weight:bold;">E-MAIL</td>
                    <td style="padding:6px;">{{ $reserva->email }}</td>
                </tr>

                <tr>
                    <td style="padding:6px; font-weight:bold;">REPRESENTANTE LEGAL</td>
                    <td style="padding:6px;">{{ $reserva->representante_legal }}</td>
                </tr>
            </table>
        @endforeach
