...
<body>
    @php
        ...
        $valorOriginalContrato = \App\Models\LoteContratado::where('contrato_id', $contrato->id)
            ->orWhere(function($query) use ($contrato) {
                if ($contrato->processo_id && $contrato->vencedor_id) {
                    $query->where('processo_id', $contrato->processo_id)
                          ->where('vencedor_id', $contrato->vencedor_id);
                }
            })->sum('valor_total');
